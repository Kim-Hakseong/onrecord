...
SALES = Respondent(role="sales_rep", quote="Yes, this is the sales rep.")
...
class TestRule1Quote:
    ...
    def test_paraphrased_quote_is_thrown_away(self, date_field, transcript):
        claim = Claim(
            ...
            quote="We'll send it on September 24th.",  # plausible, and not in the transcript
            source="spanner",
        )
        row = adjudicate_one(date_field, claim, transcript)
        assert row.verdict is Verdict.UNRESOLVED
...
class TestRule2Type:
    ...
    def test_enum_outside_the_declared_values_is_unresolved(self, transcript):
        ...
        claim = Claim(
            field=spec.name, raw_value="the boss's mood", quote="Thank you.", source="spanner"
        )
    ...
    def test_bool_and_int_parse(self, transcript):
        ...
        row = adjudicate_one(
            flag, Claim(field=flag.name, raw_value="yes", quote="Thank you.", source="spanner"), transcript
        )
        ...
        row = adjudicate_one(
            count, Claim(field=count.name, raw_value="1,200 units", quote="Thank you.", source="spanner"), transcript
        )
...
class TestRule3Authority:
    ...
    def test_role_must_be_quoted_to_count(self, transcript):
        ...
        established = establish_respondent(
            declared, "sales_rep", "Yes, this is the sales rep.", transcript=transcript
        )
        assert established.role == "sales_rep"

        unquoted = establish_respondent(
            declared, "sales_rep", "I run the sales team.", transcript=transcript
        )
        assert unquoted.role == "unknown"

        undeclared = establish_respondent(
            declared, "ceo", "Yes, this is the sales rep.", transcript=transcript
        )
...
class TestAdjudicatorContract:
    ...
    def test_conflicting_settled_claims_do_not_settle(self, date_field, transcript):
        rows = adjudicate_field(
            date_field,
            [
                Claim(field=date_field.name, raw_value="2026-09-24", quote="It ships on September 24th.", source="spanner"),
                Claim(field=date_field.name, raw_value="2026-09-30", quote="It ships on September 24th.", source="calle"),
            ],
            transcript=transcript,
            respondent=SALES,
            reference_date=REF,
